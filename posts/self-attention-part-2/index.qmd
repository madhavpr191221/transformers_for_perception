...
subtitle: "Scaled dot-product attention, multi-head attention"
...
construction intentionally. This post builds it. 
...
$$W_Q^{(h)} \in \mathbb{R}^{d_k \times D}, \qquad 
W_K^{(h)} \in \mathbb{R}^{d_k \times D}, \qquad 
W_V^{(h)} \in \mathbb{R}^{d_v \times D}$$

and produces its own query, key, and value matrices:

$$Q^{(h)} = XW_Q^{(h)\top} \in \mathbb{R}^{N \times d_k}, \qquad 
K^{(h)} = XW_K^{(h)\top} \in \mathbb{R}^{N \times d_k}, \qquad 
V^{(h)} = XW_V^{(h)\top} \in \mathbb{R}^{N \times d_v}$$

and its own attention output:

$$Y^{(h)} = \operatorname{softmax}_{\mathrm{row}}\left(\frac{Q^{(h)}{K^{(h)}}^\top}
...
![Multi-head self-attention. Each head produces its own output matrix $Y^{(h)} \in \mathbb{R}^{N \times d_v}$. The head outputs are concatenated column-wise and projected back into the original $D$-dimensional patch space.](images/multi-head-attention.png){#fig-multi-head-attention}

### Why am I not showing an attention heatmap 

I originally thought of adding an interactive attention heatmap here, which shows the relative relevance of patches. But there is a trap.
Unless the projection matrices have been learned from a well-defined task, the heatmap is not meaningful. With random $W_Q$, $W_K$, and $W_V$, the attention matrix only shows random dot-product geometry passed through softmax.
...
Images have their own geometry. Gradient-based learning adjusts the matrices until the right patches start to matter. 
And only through this learned geometry does a boat patch know that nearby water matters.
...
![What's Coming Next](images/what_comes_next.png){#fig-what_comes_next}
